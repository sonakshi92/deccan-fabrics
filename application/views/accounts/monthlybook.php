<div class="content-wrapper">
	<!-- Content Header (Page header) -->
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1><?php echo  explode('|',trim($title))[0]; ?></h1>
				</div>
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="#"> Investment and Expenses </a></li>
						<li class="breadcrumb-item active"> <?php echo date('d/m/Y') ?></li>
					</ol>
				</div>
			</div>
		</div><!-- /.container-fluid -->
		<div class="row">
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h3 class="card-title">Investment:
						<div class="card-body">
							<div class="row">
								<div class="col-md-4">
								<div class="form-group">
									<label>Investment Reason</label>
									<input type="text" class="form-control" name="c_reason" size="50">
								</div>
								</div>
								<div class="col-md-4">
								<div class="form-group">
									<label>Rs.</label>
									<input type="text" class="form-control" name="C_rs" size="50">
								</div>
								</div>
								<div class="col-md-4">
								<div class="form-group"> <br>
									<button class="form btn btn-info fa fa-plus"> Add </button>
								</div>
								</div>
								<div class="col-md-4">
								<div class="form-group">
									<label>Expenditure Reason</label>
									<input type="text" class="form-control" name="d_reason" size="50">
								</div>
								</div>
								<div class="col-md-4">
								<div class="form-group">
									<label> Rs. </label>
									<input type="text" class="form-control" name="d_rs" size="50">
								</div>
								</div>
								<div class="col-md-4">
								<div class="form-group"> <br>
									<button class="form btn btn-primary fa fa-plus"> Add </button>
								</div>
								</div>
							</div>
						</div>
					</div>
					<div class="container-fluid">
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<div class="card-body">
										<table id="example2" class="table table-striped table-bordered table-hover">
											<thead class="table table-dark">
												<tr>
													<th>Investment</th>
													<th>Rs.</th>
													<th>Action</th>
												</tr>
											</thead>
											<tbody>
												<tr>
													<td><b>Brought Forward</b></td>
													<td><b>Rs.<?php //should be closing balance of previous year?></b></td>
													<td></td>
												</tr>
												<tr>
													<td>Monthy sale</td>
													<td>5,00,000</td>
													<td><i class="fas fa-edit" style="color:blue"></i> <i
															class="fas fa-trash" style="color:red"></i> </td>
												</tr>
												<tr>
													<td>Chit Funds</td>
													<td>8,00,000</td>
													<td><i class="fas fa-edit" style="color:blue"></i> <i
															class="fas fa-trash" style="color:red"></i> </td>
												</tr>
												<tr>
													<td><b>Total</b></td>
													<td><b>5,80,000 </b></td>
													<td> </td>
												</tr>
											</tbody>
										</table>
									</div>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<table id="example2" class="table table-striped table-bordered table-hover">
										<thead class="table table-dark">
											<br>
											<tr>
												<th>Expenditure</th>
												<th>Rs.</th>
												<th>Action</th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td>Salaries</td>
												<td>15,000 </td>
												<td> <?php ?> <i class="fas fa-trash" style="color:red"></i> </td>
											</tr>
											<tr>
												<td>Food</td>
												<td>1500</td>
												<td><?php ?> <i class="fas fa-trash" style="color:red"></i></td>
											</tr>
											<tr>
												<td>Travel</td>
												<td>15000</td>
												<td><?php ?> <i class="fas fa-trash" style="color:red"></i></td>
											</tr>
											<tr>
												<td>Festival</td>
												<td>5000</td>
												<td><?php ?> <i class="fas fa-trash" style="color:red"></i></td>
											</tr>
											<tr>
												<td><b>Carry Forward</b></td>
												<td><b> </b></td>
												<td></td>
											</tr>
											<tr>
												<td><b> Total </b></td>
												<td><b>5500 </b></td>
												<td> </td>
											</tr>
										</tbody>
									</table>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
</div>

<!-- /.card-body -->
<!-- /.card -->
